...
import finalizePaymentRoutes, { type TSectionName } from '../../app/router/finalizePaymentRoutes';
...
export default function CheckoutForm({ secret }: TClientSecret) {
  ...
  const finalizePaymentRoute = encodeURI(
    finalizePaymentRoutes[redirectPath]
      .split('/:')
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      switch (paymentIntent?.status) {
        case 'succeeded':
          ...
          navigate(finalizePaymentRoute, { replace: true });
          break;
        case 'processing':
